...
It will make an output file called `<schema_name>-<org>-measures.csv` for each
organization specified in the schema, with filtered data that is ready to
share.

In the above example, the input filename has to equal the table name in the
schema. The table name can also be specify explicitly, to allow other
filenames:

```bash
odm-share --tables=measures schema.csv my_measures.csv
```

Multiple CSV files can also be used as input:

```bash
odm-share --tables=measures,samples schema.csv a.csv b.csv
```
...
- Specify tables associated with CSV input files:
