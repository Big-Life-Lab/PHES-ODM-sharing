...
3. `group`: This defines activities for a rule that groups or combines rules together for execution. The full rule will require the `key` and `operator` columns to be fully specified.
...
```
select measure, value, unit
from measures
where ((4 and 5) or (6 and 7)) and 11
```
